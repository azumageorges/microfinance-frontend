...
final Provider<ClientRepository> clientRepositoryProvider = Provider<ClientRepository>(
  ...
  (ref) => ClientRepository(
    ...
    // syncService retiré : appelait onSyncCompleted → ref.invalidate(clientsProvider)

    // depuis getClients() → loader infini (cycle de dépendance).

    // La sync est déclenchée exclusivement par SyncService.startListening().
